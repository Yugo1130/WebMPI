...
int MPI_Send(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm) {
    ...
    printf("[Rank %d] Sending %d bytes to rank %d (type: %d)\n", comm.commrank, bytes, dest, mpi_get_id(datatype));
    // 通信処理はここに追加（JS経由）
    return MPI_SUCCESS;
}
...
int MPI_Recv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm) {
    int bytes = count * mpi_get_size(datatype);
    printf("[Rank %d] Receiving %d bytes from rank %d (type: %d)\n", comm.commrank, bytes, source, mpi_get_id(datatype));
    // 通信処理はここに追加（JS経由）
    return MPI_SUCCESS;
}
